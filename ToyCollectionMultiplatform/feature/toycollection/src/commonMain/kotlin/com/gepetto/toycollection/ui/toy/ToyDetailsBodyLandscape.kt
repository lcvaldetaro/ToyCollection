...
@Composable
fun ToyDetailsBodyLandscape (
    toy: Toy,
    modifier: Modifier = Modifier,
) {
    Column (modifier.background(sysBackgroundColor())) {
        Row {
            ToyImage(toy, modifier = Modifier.weight(5f))
            ToyWrittenDetailsBody(toy, modifier = Modifier.weight(5f))
        }
    }
}

@Composable
private fun ToyWrittenDetailsBody(
    toy: Toy,
    modifier: Modifier = Modifier,
) {
    Column (modifier) {
